...
//////////////// Get Publishers from Local Storage ////////////////////////
// our version of processTrackersFromLocalStore
function getPerPublisherValue(key) {
	return getPublisherFromLocalStore(key);
}
...
function getPublishers(){
	...
	result.push("cnn.com");
	result.push("time.com");
	result.push("yahoo.com");

	for(var domain in localStorage){
		if(domain.substr(0,8) == 'publishers:'){
			var publisherURL = domain.substr(10, domain.length-9);
			//var json = JSON.parse(localStorage[domain]);
			result.push(publisherUrl)
		}
	} 
...
function getPublisherFromLocalStore(tracker){
	//if(DEBUG) console.log('getTrackerFromLocalStore ' + tracker);
	var price = 22;
	//var price = localStorage.publisher["publisher:"+tracker]
	return price;
}
...
var demographics = {
	...
	getPerPublisherValue: getPerPublisherValue
};
